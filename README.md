...
- **Build**: installs dependencies and runs `collectstatic` (served by
  WhiteNoise).
- **Pre-deploy**: runs migrations, then loads `fixtures/initial_data.json`
  so the demo accounts and listings described above are present after every
  deploy.
...
- The database is SQLite on the service's local disk, which is **not**
  persisted across deploys or restarts - that's why the pre-deploy step
