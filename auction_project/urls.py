from django.contrib import admin
from django.conf import settings
from django.urls import path, include
from django.views.static import serve as serve_static
from pages import views as pages_views

admin.site.site_header = 'Online Auction Administration'
admin.site.site_title = 'Online Auction Admin'
admin.site.index_title = 'Site Administration'

urlpatterns = [
    path('', pages_views.home, name='home'),
    path('contact/', pages_views.contact_submit, name='contact'),
    path('about/', pages_views.about, name='about'),
    path('team/', pages_views.team, name='team'),
    path('privacy/', pages_views.privacy, name='privacy'),
    path('terms/', pages_views.terms, name='terms'),
    path('admin/', admin.site.urls),
    path('accounts/', include('accounts.urls')),
    path('listings/', include('listings.urls')),
    path('bids/', include('bids.urls')),
    path('timers/', include('timers.urls')),
    path('messages/', include('conversations.urls')),
    path('notifications/', include('notifications.urls')),
    path('reviews/', include('reviews.urls')),
]

# django.conf.urls.static.static() is a no-op outside DEBUG, which would
# leave uploaded listing photos and ID documents 404ing in production.
# There's no object storage (S3, Cloudinary, etc.) configured, so serve
# MEDIA_ROOT directly regardless of DEBUG - acceptable for a small,
# single-instance deployment without heavy media traffic.
urlpatterns += [
    path('media/<path:path>', serve_static, {'document_root': settings.MEDIA_ROOT}),
]
